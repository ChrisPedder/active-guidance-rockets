...
class OpenRocketParser:
        ...
        try:
            with zipfile.ZipFile(ork_path, "r") as zf:
                ...
                for name in names:
                    ...
                    if name == "rocket.ork" or (
                        name.endswith(".ork") and "/" not in name
                    ):
                        rocket_file = name
                        break
    ...
    def _parse_subcomponents(
        cls, parent: ET.Element, components: List[Component], position: float
    ):
        ...
        for elem in parent:
            ...
            if tag == "nosecone":
                ...
                inner_subcomps = cls._find_element(
                    elem, "subcomponents", "{*}subcomponents"
                )
                if inner_subcomps is not None:
                    cls._parse_bodytube_contents(
                        inner_subcomps, components, current_position
                    )
                ...
                stage_subcomps = cls._find_element(
                    elem, "subcomponents", "{*}subcomponents"
                )
                if stage_subcomps is not None:
                    cls._parse_subcomponents(
                        stage_subcomps, components, current_position
                    )

    @classmethod
    def _parse_bodytube_contents(
        cls, parent: ET.Element, components: List[Component], bt_position: float
    ):
