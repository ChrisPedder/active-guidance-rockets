...
def get_motor(motor_name: str):
    ...
    raise ValueError(
        f"No config found for motor: {motor_name}. "
...
def plot_motor_profile(motor, save_path: str = None, show: bool = True):
    ...
    fig.suptitle(
        f"{motor.manufacturer} {motor.designation} Motor Profile",
        fontsize=16,
        fontweight="bold",
    )
...
def plot_motor_comparison(motor_names: list, save_path: str = None, show: bool = True):
    ...
    ax4.bar(
        x - width, impulses, width, label="Total Impulse (N·s)", color="blue", alpha=0.7
    )
...
Examples:
    ...
    parser.add_argument("--compare", nargs="+", help="Compare multiple motors")
    ...
    if args.compare:
        ...
        plot_motor_comparison(
            ["estes_c6", "aerotech_f40", "cesaroni_g79"],
            save_path=args.save,
